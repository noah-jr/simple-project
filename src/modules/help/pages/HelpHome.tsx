...
import { useHelpCategories } from '../queries/useHelpQueries';

export const HelpHome = () => {
  const { data: categories = [], isLoading } = useHelpCategories();

  useEffect(() => {
    document.title = 'Base de Conhecimento e Suporte | PetroHost';
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans">
      ...
      <main className="flex-grow w-full max-w-[960px] mx-auto px-4 md:px-[15px] py-12">
        {isLoading ? (
          <div className="flex justify-center py-10 text-gray-400">A carregar categorias...</div>
        ) : (
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-[#f4f5ff] rounded-xl p-8 border border-[#e2dbfc] hover:border-[#5025d1] transition-colors cursor-pointer group">
              <h3 className="text-[20px] font-bold text-[#331c74] mb-3">Academia Petrohost</h3>
              ...
              <span className="text-[#331c74] font-semibold text-sm">Aceder à Academia →</span>
            </div>
            
            <div className="bg-[#f4f5ff] rounded-xl p-8 border border-[#e2dbfc] hover:border-[#5025d1] transition-colors cursor-pointer group">
              <h3 className="text-[20px] font-bold text-[#331c74] mb-3">Tutoriais em Vídeo</h3>
              ...
              <span className="text-[#331c74] font-semibold text-sm">Ver Tutoriais →</span>
            </div>
          </div>
        </section>
      </main>
